...
\hypersetup{
  pdftitle={¿El modelo mira el balón o mira el césped?},
  pdfauthor={Andy Fuentes, Diego Linares}
}

\title{\textbf{\textquestiondown{}El modelo mira el balón o mira el césped?}\\[2mm]
\large Una auditoría con SHAP de un clasificador ViT sobre escenas de fútbol}

\author{Andy Fuentes y Diego Linares\\
...
\textquestiondown{}qué parte de la imagen sostiene esa respuesta?
...
\begin{table}[H]
\centering
\input{generado/tabla_predicciones.tex}
...
\centering
\small
\input{generado/tabla_metricas.tex}
...
\caption{Ablaciones. Izquierda: solo se conserva la caja del balón
(\textquestiondown{}basta el objeto?). Derecha: solo se borra la caja del balón
(\textquestiondown{}es necesario?). En A y B el objeto manda; en C
...
python verify_submission.py             # Validar el paquete antes de entregarlo
cd paper
pdflatex paper.tex
bibtex paper
pdflatex paper.tex
...
El verificador \texttt{verify\_submission.py} comprueba además, sin descargar nada,
que el paquete conserva todos los entregables, que los JSON y el cuaderno son válidos,
que los hashes de las imágenes coinciden y que cada recurso citado por LaTeX existe.
